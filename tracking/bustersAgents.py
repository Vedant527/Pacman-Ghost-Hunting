...
class GreedyBustersAgent(BustersAgent):
    ...
    def chooseAction(self, gameState):
        ...
        for dist in livingGhostPositionDistributions: # iterate through ghosts
            position = dist.argMax()
            distance = self.distancer.getDistance(pacmanPosition, position)
            if distance < min: # compare ghost positions relative to Pac
                min = distance
                ghostPos = position # reassign ghost position/dist


        bestAction = None
        bestDist = float("inf")

        for action in legal: # consider all actions
            successor = Actions.getSuccessor(pacmanPosition, action)
            distance = self.distancer.getDistance(successor, ghostPos) # find distance of next position compared to closest ghost

            if distance < bestDist: # compare new distance to previous best distance to determine best action
                bestDist = distance
                bestAction = action
