...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        """
        "*** YOUR CODE HERE ***"
        update = self.beliefs.copy() # original array of probabilities
        noisyDistance = observation
        pacmanPosition = gameState.getPacmanPosition()
        # The probability of the observation

        for ghostPosition in self.allPositions:
            emissionProb = self.getObservationProb(observation, gameState.getPacmanPosition(), # P(noisyDistance | pacmanPosition, ghostPosition)
                                                   ghostPosition, self.getJailPosition())
            update[ghostPosition] = emissionProb * self.beliefs[ghostPosition] # update proability based on old probability

        self.beliefs = update # updated array of probabilities
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation, gameState):
        ...
        for part in self.particles:
            ...
            tmp[part] += prob # update probability based on old prob
        if tmp.total() == 0:
    ...
    def elapseTime(self, gameState):
        ...
        for i in range(self.numParticles):
            ...
            if particle in update:
                self.particles[i] = update[particle].sample()
            else:
                tmp = self.getPositionDistribution(gameState, particle)
                update[particle] = tmp
    ...
    def getBeliefDistribution(self):
        ...
        for part in self.particles:
            dist[part] += 1 # weight for potential position
